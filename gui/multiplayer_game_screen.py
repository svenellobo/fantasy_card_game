...
class MultiplayerGameScreen(ctk.CTkFrame):
    def __init__(self, parent, mp_game):
        super().__init__(parent)
        self.parent = parent
        self.mp_game = mp_game
    ...
    def init_screen(self): 
        ...
        #current opponent turn
        self.current_player = ctk.CTkFrame(self, height=240, fg_color="#6D4C41") 
        self.current_player.grid(row=0, column=1, sticky="ew", padx=10, pady=10)
        self.current_player.grid_propagate(False)

        self.current_player_lbl = ctk.CTkLabel(self.current_player, text="", font=("Verdana Arial", 14, "bold"),
                                           text_color="orange")
        self.current_player_lbl.grid(row=0, column=0, sticky="ew", padx=10, pady=10)
    ...
    def open_score_screen(self, player1, player2, discard_area):
        self.destroy()        
        score_screen = ScoreScreen(self.parent, player1, player2, discard_area, self.mp_game.image_paths)
        score_screen.grid(row=0, column=0, sticky="nsew")
        
    def open_choice_screen(self, player1, player2, discard_area):
        self.destroy()
        player_choice_screen = PlayerChoiceScreen(self.parent, player1, player2, discard_area, self.mp_game.image_paths)
